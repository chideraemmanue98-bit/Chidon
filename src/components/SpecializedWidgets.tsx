...
export const GoogleBrowserEngineWidget = () => {
  ...
  // Fetch backend autonomous crawler status and cache metrics
  const fetchCrawlerStatus = async () => {
    try {
      ...
      console.error("Failed to fetch backend crawler status", err);
    }
  };
  ...
  useEffect(() => {
    handleCrawl(true);
    fetchCrawlerStatus();
    const interval = setInterval(fetchCrawlerStatus, 15000); // refresh every 15s
    return () => clearInterval(interval);
  }, []);

  const triggerBackendCrawler = async () => {
    try {
      setTerminalLogs(prev => [...prev, "[SYS] Manual re-trigger command sent to background crawler daemon..."]);
      const res = await fetch("/api/crawler/trigger", { method: "POST" });
      const data = await res.json();
      if (data.success) {
        setTerminalLogs(prev => [...prev, "[SUCCESS] Background crawler daemon reactivated! Google Search scanning in progress..."]);
        setTimeout(fetchCrawlerStatus, 1000);
      } else {
        setTerminalLogs(prev => [...prev, `[WARNING] Crawler trigger response: ${data.message || "active"}`]);
      }
    } catch (err: any) {
      setTerminalLogs(prev => [...prev, `[FAIL] Failed to trigger background crawler daemon: ${err.message}`]);
    }
  };
  ...
  const handleCrawl = async (initialRun = false) => {
    ...
    const logs = [
      ...
      "[SYS] Querying Chidon IQ pre-crawled daily cache...",
    ];
      ...
      setTimeout(() => {
        if (data.success && data.videos) {
          ...
          setTerminalLogs(prev => [
            ...prev,
            `[SYS] Navigation success: Loaded ${data.videos.length} assets from Chidon IQ index.`,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>{loading ? "Crawling..." : "Refresh Index"}</span>
          </button>
        </div>
      </div>

      {/* AUTONOMOUS BACKGROUND DAEMON DIAGNOSTICS */}
      {crawlerStatus && (
        <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-mono">
          <div className="flex items-center gap-2.5">
            <div className="relative flex h-2.5 w-2.5">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${crawlerStatus.status === "running" ? "bg-cyan-400" : "bg-emerald-400"}`}></span>
              <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${crawlerStatus.status === "running" ? "bg-cyan-500" : "bg-emerald-500"}`}></span>
            </div>
            <div>
              <span className="text-slate-500 block text-[9px] uppercase font-black">AUTO CRAWLER DAEMON</span>
              <span className="text-white font-extrabold uppercase tracking-wide">{crawlerStatus.status === "running" ? "SEARCHING GOOGLE..." : "MONITORING DAILY"}</span>
            </div>
          </div>
          <div>
            <span className="text-slate-500 block text-[9px] uppercase font-black">LAST SEARCH RUNTIME</span>
            <span className="text-slate-300 font-extrabold">
              {crawlerStatus.lastRun === "never" ? "Auto-seeding cache..." : new Date(crawlerStatus.lastRun).toLocaleTimeString()}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <div>
              <span className="text-slate-500 block text-[9px] uppercase font-black">SYNCHRONIZED NICHE INDEX</span>
              <span className="text-cyan-primary font-black">{crawlerStatus.totalCachedKeys} Niche Bundles</span>
            </div>
            <button 
              onClick={triggerBackendCrawler}
              disabled={crawlerStatus.status === "running"}
              className="px-2.5 py-1 bg-slate-950 border border-white/10 hover:border-cyan-primary/40 text-cyan-primary rounded-lg text-[9px] font-mono font-bold uppercase tracking-wider transition-all disabled:opacity-40 cursor-pointer"
            >
              Trigger Crawl
            </button>
          </div>
