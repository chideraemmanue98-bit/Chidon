...
import { 
  Briefcase, DollarSign, Send, User, Check, Activity, Layers, Zap, 
  Clock, ArrowRight, Search, AlertCircle, Trash2, Plus, ShoppingBag, 
  TrendingUp, Coins, Shield, HelpCircle, FileText, CheckCircle, ChevronLeft,
  Mail, Globe, Star, ExternalLink
} from 'lucide-react';
...
export const EarnSection: React.FC<EarnSectionProps> = ({ onBack, user, onSignIn }) => {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'buyer' | 'seller'>('dashboard');
  ...
  const handleBuyService = async (service: EarnService) => {
    ...
    const confirmBuy = window.confirm(`Initiate purchase order with candidate for "${service.title}" at $${service.price}?`);
    if (!confirmBuy) return;

    try {
      // Create a direct job contract pre-assigned to this seller
      await addDoc(collection(db, 'earn_jobs'), {
        title: `Service Order: ${service.title}`,
        description: `Direct purchase contract for specialty package. Instructions: ${service.description}`,
        budget: service.price,
        requirements: `Fulfill custom deliverables and provide execution link. Delivery Speed: ${service.deliveryTime}`,
        buyerId: user.uid,
        buyerEmail: user.email || 'Anonymous Buyer',
      ...
      alert("Purchase order dispatched securely onto candidate's workflow ledger! The seller will receive notification and deliver work deliverables.");
    } catch (err) {
      console.error("Purchase order failed:", err);
    }
  };
  ...
  return (
    <div className="w-full min-h-screen bg-[#070A13] text-[#F1F5F9] font-sans pb-16 relative overflow-hidden selection:bg-[#22D3EE]/30">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-8 mb-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-black uppercase tracking-tight font-sans leading-none flex items-center gap-2">
                  CHIDON EARN <span className="text-[10px] bg-[#22D3EE]/10 border border-[#22D3EE]/30 text-[#22D3EE] font-mono px-2 py-0.5 rounded uppercase font-bold tracking-widest">v1.1</span>
                </h1>
                <p className="text-xs text-slate-400 font-mono mt-1 uppercase tracking-widest">
                  Secure Cognitive Gigs & Services Synced to Live Ledger
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 bg-[#0F172A] border border-white/10 p-3.5 rounded-2xl md:self-end">
            ...
            <div className="text-xs">
              {user ? (
                <div>
                  <span className="text-[10px] text-slate-500 font-mono block uppercase">Active Sync account</span>
                  <span className="font-bold text-slate-200">{user.email}</span>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  <span className="text-slate-400">Offline state (limited read).</span>
                  <button 
                    onClick={onSignIn}
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 max-w-2xl mx-auto mb-10 p-1.5 bg-[#0F172A]/80 border border-white/5 rounded-2xl">
          <button
                ...
                <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                  <div className="space-y-4 max-w-2xl">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 text-[9px] font-mono tracking-widest text-[#22D3EE] bg-[#22D3EE]/10 border border-[#22D3EE]/25 rounded-full uppercase font-extrabold shadow-sm animate-pulse">
                      <Zap size={10} className="text-[#22D3EE]" /> CRATER HUB: EXCLUSIVELY FOR SOCIAL MEDIA GIG SERVICES
                    </span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                  <div className="p-6 bg-[#161226] border border-white/10 rounded-3xl shadow-xl space-y-5">
                    <div className="flex items-center gap-2 border-b border-white/5 pb-4">
                      <Zap className="text-[#A78BFA] animate-pulse" size={18} />
                              ...
                              >
                                {job.sellerId && (
                                  <div className="text-[9px] bg-indigo-500/10 border border-indigo-500/25 text-indigo-300 font-mono px-2 py-1 rounded w-fit uppercase font-bold tracking-widest flex items-center gap-1">
                                    <Zap size={10} className="text-indigo-400" /> DIRECT SERVICE ORDER FOR YOU
                                  </div>
                                )}
