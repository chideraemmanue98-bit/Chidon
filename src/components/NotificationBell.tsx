...
import { 
  Bell, Coins, MessageSquare, Zap, Info, Check, Trash2, ExternalLink 
} from 'lucide-react';
...
export const NotificationBell: React.FC<NotificationBellProps> = ({ 
  ...
  const getNotificationIcon = (type: NotificationItem['type']) => {
    switch (type) {
      ...
      case 'ai_result':
        return <Zap size={14} className="text-purple-400" />; // Purple/Orange
      case 'system':
      default:
              ...
              {notifications.length === 0 ? (
                ...
                notifications.slice(0, 10).map((item) => (
                  ...
                  >
                    ...
                    {/* Unread dot / Mark as Read / Delete button */}
                    <div className="flex flex-col items-end justify-between self-stretch shrink-0 pl-1">
                      {!item.isRead ? (
                        <button
                          ...
                          className="p-1 text-indigo-400 hover:text-indigo-300 rounded transition-all cursor-pointer bg-transparent border-none flex items-center justify-center"
                          title="Mark as read"
                          id={`mark-read-bell-btn-${item.id}`}
                        >
