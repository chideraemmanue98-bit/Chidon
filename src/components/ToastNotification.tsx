...
import { Coins, MessageSquare, Zap, Info, X } from 'lucide-react';
...
export const ToastNotification: React.FC = () => {
    ...
    if (notifications.length > 0) {
      ...
      if (!latest.isRead && latest.id !== lastId) {
        ...
        toast.custom(
            ...
            <AnimatePresence mode="wait">
              {t.visible && (
                <motion.div
                  ...
                  className={`max-w-sm w-full bg-slate-950 border ${getBorderColor(latest.type)} rounded-2xl p-4 shadow-2xl flex items-start gap-3 backdrop-blur-md pointer-events-auto`}
                  onClick={() => {
                    toast.dismiss(t.id);
                    if (latest.link) {
          ...
          {
            duration: 15000, // Extended to 15 seconds to ensure ample time to read
  ...
  const getIcon = (type: NotificationItem['type']) => {
    switch (type) {
      ...
      case 'ai_result':
        return <Zap size={14} className="text-emerald-400" />; // success = green
      case 'system':
      default:
